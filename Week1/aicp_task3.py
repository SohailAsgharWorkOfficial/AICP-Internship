...
if additional_items:
    ...
    print("New Total Price (Before Discount): ${:.2f}".format(total_price))
    ...
    print("New Total Price (After Discount): ${:.2f}".format(total_price))
else:
    print("No additional items selected. The total price remains ${:.2f}".format(total_price))

def print_receipt(chosen_case, chosen_ram, chosen_hdd, additional_items, total_price, item_data):
    print("\nReceipt:")
    
    receipt_table = PrettyTable()
    receipt_table.field_names = ["Description", "Item code", "Price ($)"]
    
    receipt_items = [
        (next((desc for code, desc, price in item_data if code == chosen_case), None), chosen_case, chosen_case_price),
        (next((desc for code, desc, price in item_data if code == chosen_ram), None), chosen_ram, chosen_ram_price),
        (next((desc for code, desc, price in item_data if code == chosen_hdd), None), chosen_hdd, chosen_hdd_price)
    ]
    
    for item in receipt_items:
        description, item_code, price = item
        receipt_table.add_row([description, item_code, price])
    
    for additional_item in additional_items:
        description = next((desc for code, desc, price in item_data if code == additional_item), None)
        price = next((price for code, desc, price in item_data if code == additional_item), None)
        receipt_table.add_row([description, additional_item, price])
...
# Print the receipt
print_receipt(chosen_case, chosen_ram, chosen_hdd, additional_items, total_price,item_data)
